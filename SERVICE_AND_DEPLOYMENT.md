...
- 월, 수, 금 운동 일정 표시
- 운동 순서: 가슴, 등, 어깨, 복근, 하체
- 홈트 장비 반영 7+2 루틴: 푸쉬업바 일반 푸쉬업, 덤벨 플로어 프레스, 밴드 랫 풀다운, 덤벨 벤트오버 로우, 밴드 사이드 레터럴 레이즈, AB슬라이드, 밴드 스쿼트, 선택 운동인 사이드 플랭크 리치스루와 밴드 글루트 브릿지
- 각 운동별 이미지, 목표 부위, 반복 횟수, 목표 세트, 휴식 시간 표시
- 운동 이미지를 클릭하면 운동 소개, 시작/움직일 때/도착/돌아올 때/느낌, 주의점, 초보자 조절법, 자세 경고, 중단 기준 표시
- 세트별 실제 수행 횟수, 무게/밴드 강도, 컨디션 입력
- 전체 운동 시간 시작/일시정지, 세트 완료, 휴식 타이머, 필수 루틴 기준 오늘 운동 완료 기록
- 다음 세트 또는 다음 운동 자동 안내
- 일별 기록, 달력 기록, 주간 요약, 연속 달성 배지, CSV/JSON 백업
- Google Sheets 동기화 상태, 실패 상세, 대기열 재시도/비우기 표시
- Google 로그인 기반 계정 연동
...
- 사용자의 장비인 푸쉬업바, 7kg 덤벨 2개, 튜빙밴드, 고리형 밴드, AB슬라이드를 기준으로 루틴 재구성
- 초보자 부담을 줄이기 위해 `필수 7종목 + 선택 2종목` 구조로 변경
- 선택 운동을 하지 않아도 필수 7종목을 최소 1세트씩 기록하면 `오늘 운동 완료` 가능
- 가슴 운동을 `푸쉬업바 일반 푸쉬업`, `덤벨 플로어 프레스` 2종으로 변경
- 어깨 운동을 7kg 덤벨 대신 `밴드 사이드 레터럴 레이즈`로 변경해 저항 조절이 쉽도록 개선
- 복근은 `AB슬라이드`를 필수, `사이드 플랭크 리치스루`를 선택으로 구성
- 하체는 `밴드 스쿼트`를 필수, `밴드 글루트 브릿지`를 선택으로 구성
- 운동 설명을 `시작`, `당길 때/밀 때/내릴 때`, `도착`, `돌아올 때`, `느낌` 중심으로 작성
- SVG 안내 그림은 사용하지 않고, 새로 필요한 이미지는 raster WebP 자산으로 추가
...
2.4.0 추가 변경 사항은 다음과 같습니다.

- 왼쪽 운동 순서를 `필수 루틴`과 `선택 추가`로 분리 표시
- 세트 완료 시 `무게/밴드 강도`와 `컨디션`을 함께 기록
- 운동 상세에 `자세 경고`와 `중단 기준` 추가
- 타이머 안내 문구에 다음 세트/다음 운동을 더 명확하게 표시
- 오른쪽 기록 패널에 이번 주 완료 횟수, 필수/선택 세트, 총 운동 시간, 불편/통증 수, 달성 배지 표시
- 동기화 실패 시 오류 상세, 마지막 실패 시각, 실패 작업, 재시도 횟수 표시
- 수동 재시도 버튼과 동기화 대기열 비우기 버튼 추가
- Apps Script `SetLogs`에 `loadText`, `condition` 컬럼 추가

## 초보자 루틴 구성

현재 앱의 기본 운동 순서는 아래와 같습니다.

| 순서 | 운동 | 구분 | 목표 반복 | 목표 세트 | 장비 |
| --- | --- | --- | --- | --- | --- |
| 1 | 푸쉬업바 일반 푸쉬업 | 필수 | 8-15회 | 3세트 | 푸쉬업바 |
| 2 | 덤벨 플로어 프레스 | 필수 | 8-12회 | 3세트 | 7kg 덤벨 2개, 매트 또는 수건 |
| 3 | 밴드 랫 풀다운 | 필수 | 10-15회 | 3세트 | 튜빙밴드, 도어앵커 |
| 4 | 덤벨 벤트오버 로우 | 필수 | 10-15회 | 3세트 | 7kg 덤벨 2개 |
| 5 | 밴드 사이드 레터럴 레이즈 | 필수 | 12-20회 | 3세트 | 튜빙밴드 |
| 6 | AB슬라이드 | 필수 | 5-10회 | 3세트 | AB슬라이드, 무릎 매트 |
| 7 | 밴드 스쿼트 | 필수 | 10-15회 | 3세트 | 고리형 밴드 |
| 8 | 사이드 플랭크 리치스루 | 선택 | 좌우 8-12회 | 3세트 | 매트 |
| 9 | 밴드 글루트 브릿지 | 선택 | 10-15회 | 3세트 | 고리형 밴드, 매트 또는 수건 |

운동 초보자는 모든 운동을 처음부터 3세트씩 채우지 않아도 됩니다. 앱은 세트 완료를 누른 기록만 저장하므로, 필수 7종목을 최소 1세트씩 수행한 뒤 선택 운동을 하지 않아도 `필수 완료로 오늘 운동 완료`를 누를 수 있습니다.
...
| `assets/pushup-bar-pushup.webp` | AI 생성 raster 이미지 | 프로젝트 내 사용 |
| `assets/dumbbell-floor-press.webp` | AI 생성 raster 이미지 | 프로젝트 내 사용 |
| `assets/band-lateral-raise.webp` | AI 생성 raster 이미지 | 프로젝트 내 사용 |
...
| syncedAt | 시트에 저장된 시각 |
| loadText | 무게 또는 밴드 강도 |
| condition | 세트 컨디션: `ok`, `hard`, `discomfort`, `pain` |
...
| totalSets | 필수 루틴 목표 세트 수 |
| completedSets | 완료된 필수 세트 수 |
...
이미 배포된 Apps Script를 2.4.0 기능으로 갱신하려면 Apps Script 편집기에서 기존 `Code.gs`를 이 저장소의 최신 `apps-script/Code.gs` 내용으로 바꾼 뒤 저장하고, `배포` > `배포 관리`에서 새 버전을 만들어야 합니다. 이 과정을 해야 `loadText`, `condition` 컬럼이 Google Sheets에 자동 추가됩니다.
...
- 전송 실패 시 오류 상세와 마지막 실패 시각, 실패 작업, 재시도 횟수를 표시합니다.
- `재시도`는 현재 대기열을 다시 전송하고, `대기열 비우기`는 아직 전송되지 않은 항목만 삭제합니다. 로컬 운동 기록은 지워지지 않습니다.
